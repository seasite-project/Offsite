...
from offsite.evaluation.kerncraft_utils import execute_kerncraft_ecm_mode, parse_kerncraft_output_ecm_mode
...
from offsite.evaluation.yasksite_utils import execute_yasksite_ecm_mode, parse_yasksite_output
...
def train_kernel_predictions(db_session: Session, machine: Machine, templates: List[KernelTemplate],
    ...
    if errors:
        db_session.rollback()
        raise RuntimeError('Failed to train kernel predictions: {}'.format(errors[0]))
...
def compute_kernel_prediction(kernel: Kernel, machine: Machine, method: ODEMethod, ivp: IVP):
    ...
    return kernel.db_id, (
        machine.db_id, machine.compiler.db_id, method.db_id, - 1 if not ivp else ivp.db_id), kernel_predictions
