"""@package impl_generator_cpp_mpi
...
class ImplCodeGeneratorCppMPI:
    ...
    folder_impl: Path
        Folder all created impl variant code files are stored in.
    folder_ivp: Path
    ...
    def generate(self, skeleton: ImplSkeleton, impl_variants: List[Tuple[int]], ivp: IVP,
        ...
        for vid, variant in impl_variants:
            ...
            if config.args.tile:
                ...
                name = Path('{}/{}_tiled.hpp'.format(self.folder_impl, variant_name))
                vid = 123456
    ...
    def generate_implementation_tree(self, node: CodeNode, skeleton: 'ImplSkeleton', method: 'ODEMethod'):
        """Generate impl variant code tree.
    ...
    def generate_impl_variant(self, impl: CodeNode, variant_id: int, kernels: List[Kernel], skeleton: ImplSkeleton,
        ...
        code: str = ImplCodeGeneratorCppMPI.generate_implementation_code(impl, kernels, method, gen_tiled_code)
        # Write frame code.
        return self.write_skeleton_includes(skeleton, method, ivp) + self.write_class(
            variant_name, variant_id, method.name, code)
    ...
    def generate_implementation_code(
        ...
        # Traverse tree depth first.
        if node.child:
            code += ImplCodeGeneratorCppMPI.generate_implementation_code(node.child, kernels, method, gen_tiled_code)
        # Close opened brackets.
        if node.type == CodeNodeType.LOOP:
            code += write_closing_bracket(node.indent)
        if node.next:
            code += ImplCodeGeneratorCppMPI.generate_implementation_code(node.next, kernels, method, gen_tiled_code)
        return code
    ...
    def write_butcher_table(method: ODEMethod) -> str:
        ...
        str
            Generated code.
        """
        butcher = ImplCodeGeneratorCppMPI.write_2d_array('A', method.coefficientsA)
        butcher += ImplCodeGeneratorCppMPI.write_1d_array('b', method.coefficientsB)
        butcher += ImplCodeGeneratorCppMPI.write_1d_array('c', method.coefficientsC)
        return butcher + '\n'
    ...
    def write_codes_to_file(generated_files: Dict[Path, str]):
        """Write generated impl variant codes to files.

        Parameters:
        -----------
        generated_files: dict (key=Path, value=str)
            Generated impl variant codes and their associated file paths.
